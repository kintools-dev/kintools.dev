// Generates public/sitemap.xml, public/llms.txt, public/llms-full.txt, and
// src/content/doc-titles.ts from form/docs and store/docs' raw markdown +
// nav.json. Unlike scripts/build-search-index.ts, this never needs to
// render MDX -- a page's frontmatter and raw markdown source are enough
// for a link index and a full-text dump, so this is a plain fs script with
...
function buildSitemap(paths: string[], lastmod: string): string {
...
// Emits src/content/doc-titles.ts: a slug -> title map the doc routes read
// for a real per-page <title> (see src/lib/seo.ts). The title is the same
// one llms.txt uses -- a page's `title:` frontmatter, else its first `# H1`.
// The project homepages (empty slug) are left out; their routes title
// themselves from nav.json directly. `main` runs `deno fmt` over the result
// so the committed file stays formatter-clean across regenerations.
function buildDocTitlesModule(
  projectPages: Record<ProjectName, Page[]>,
): string {
  const slugToTitle = (pages: Page[]) =>
    Object.fromEntries(
  ...
  const map = {
    form: slugToTitle(projectPages.form),
    store: slugToTitle(projectPages.store),
  };

  return `// Generated by scripts/build-geo.ts. Do not edit by hand.
//
...
async function main() {
  ...
  await writeFile(`${outDir}/sitemap.xml`, buildSitemap(allPaths, lastmod));
  await writeFile(
    `${outDir}/llms.txt`,
  ...
  const docTitlesPath = fileURLToPath(
    new URL("../src/content/doc-titles.ts", import.meta.url),
  );
  await writeFile(docTitlesPath, buildDocTitlesModule(projectPages));
  await new Deno.Command(Deno.execPath(), {
    args: ["fmt", "--quiet", docTitlesPath],
    stdout: "null",
    stderr: "null",
  }).output();
  ...
  const counts = projects.map(({ name }) =>
    ...
    .join(", ");
  console.log(
    `Wrote public/sitemap.xml, public/llms.txt, public/llms-full.txt, ` +
      `src/content/doc-titles.ts (${counts} pages)`,
  );
}
